...
double CFctT::G(int j, vector<double>& p_eta, vector<double>& Q, double beta)
{
...
double CFctP::G(int j, vector<double>& p_eta, vector<double>& Q, double beta)
{
    const double    g = 1.0;
    double          ret = 0.0;
...
void CMDLoop::CalcInitialForces(CSimulationBox& SimBox, vector<CMDForces>& F)
{
...
void CMDLoop::AppendToXYZFile(vector<CParticle3D>& aParticles, int t)
{
...
void CMDLoop::ResizeDistrArrays(vector<int>* aMomentumDistr, vector<int>& aVolumeDistr,
...
void CMDLoop::AppendToVolumeDistribution(double V, vector<int>& aVolumeDistr, double dMaxV)
{
    int i = int((V / dMaxV) * double(aVolumeDistr.size()));
    if(i < (int)aVolumeDistr.size()) aVolumeDistr[i]++;
}

void CMDLoop::StoreMomentumDistribution(string szFileName,
                                              vector<int>& aMomentumDistr,
                                              double dMaxP, int iAxis)
{
    ...
    string szUnit = (iAxis < 3) ? "p[gÅ/(mol*fs)]" : "p[kJ*fs/mol]";
...
void CMDLoop::StoreVolumeDistribution(string szFileName, vector<int>& aVolumeDistr, double dMaxV)
{
    FILE*       pFile = fopen(szFileName.data(), "w");
...
void CMDLoop::UpdateOutput(int t, int iEquilibSteps, int iOutputEvery, CSimulationBox& SimBox,
                           const vector<CMDForces>& F, vector<int>* aMomentumDistr, vector<int>& aVolumeDistr)
{
...
void CMDLoop::FinalizeOutput(CSimulationBox& SimBox, vector<int>* aMomentumDistr, vector<int>& aVolumeDistr)
{
