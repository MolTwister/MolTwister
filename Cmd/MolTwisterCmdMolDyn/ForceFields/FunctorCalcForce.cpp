...
CMDForces CFunctorCalcForce::operator()(CMDFFMatrices::CAtom& atom)
{
    ...
    if(fabs(F.x_) > cutF_) { F.x_ = ((F.x_ >= 0.0) ? 1.0 : -1.0) * cutF_; lastErrorList_[k].lastWarningCode_ = CMDFFMatrices::CLastError::warnForcesWereCut; }
    if(fabs(F.y_) > cutF_) { F.y_ = ((F.y_ >= 0.0) ? 1.0 : -1.0) * cutF_; lastErrorList_[k].lastWarningCode_ = CMDFFMatrices::CLastError::warnForcesWereCut; }
    if(fabs(F.z_) > cutF_) { F.z_ = ((F.z_ >= 0.0) ? 1.0 : -1.0) * cutF_; lastErrorList_[k].lastWarningCode_ = CMDFFMatrices::CLastError::warnForcesWereCut; }
...
C3DVector CFunctorCalcForce::calcForceNonBondedOn_r_k(const C3DVector& r_k, const C3DVector& r_i, const int& k, const int& i)
{
    ...
    size_t ffCount = nonBondFFMatrixFFCount_[CMDFFMatrices::toIndexNonBond(atomList_[k].typeIndex_, atomList_[i].typeIndex_, Natomtypes_)];
    for(int ffIndex=0; ffIndex<(int)ffCount; ffIndex++)
    {
        ...
        CMDFFMatrices::CPoint p_first = nonBondFFMatrix_[CMDFFMatrices::toIndexNonBond(atomList_[k].typeIndex_, atomList_[i].typeIndex_, ffIndex, 0, Natomtypes_, Natomtypes_, MAX_FF_PER_ATOMIC_SET)];
        CMDFFMatrices::CPoint p_last = nonBondFFMatrix_[CMDFFMatrices::toIndexNonBond(atomList_[k].typeIndex_, atomList_[i].typeIndex_, ffIndex, NUM_POINTS_IN_PROFILES-1, Natomtypes_, Natomtypes_, MAX_FF_PER_ATOMIC_SET)];
        ...
        {
            CMDFFMatrices::CPoint p1 = nonBondFFMatrix_[CMDFFMatrices::toIndexNonBond(atomList_[k].typeIndex_, atomList_[i].typeIndex_, ffIndex, idx, Natomtypes_, Natomtypes_, MAX_FF_PER_ATOMIC_SET)];
            CMDFFMatrices::CPoint p2 = nonBondFFMatrix_[CMDFFMatrices::toIndexNonBond(atomList_[k].typeIndex_, atomList_[i].typeIndex_, ffIndex, idx+1, Natomtypes_, Natomtypes_, MAX_FF_PER_ATOMIC_SET)];
...
C3DVector CFunctorCalcForce::calcForceBondOn_r_k(const C3DVector& r_k, const C3DVector& r_i, const int& bondType)
{
    ...
    // Use number of points in plot to get first and last value, thus obtaining r_min and r_max, which
    // is to be used to estimate delta_r, hence enabling calculation of the closest index, j, below r
    CMDFFMatrices::CPoint p_first = bondFFList_[CMDFFMatrices::toIndexBonded(bondType, 0,NUM_POINTS_IN_PROFILES)];
    CMDFFMatrices::CPoint p_last = bondFFList_[CMDFFMatrices::toIndexBonded(bondType, NUM_POINTS_IN_PROFILES-1,NUM_POINTS_IN_PROFILES)];
    float delta = (p_last.x_ - p_first.x_) / float(NUM_POINTS_IN_PROFILES);
    float r = (r_k - r_i).norm();
    int idx = int((r - p_first.x_) / delta);
    ...
    if((idx >= 0) && (idx < (NUM_POINTS_IN_PROFILES-1)))
    {
        CMDFFMatrices::CPoint p1 = bondFFList_[CMDFFMatrices::toIndexBonded(bondType, idx,NUM_POINTS_IN_PROFILES)];
        CMDFFMatrices::CPoint p2 = bondFFList_[CMDFFMatrices::toIndexBonded(bondType, idx+1,NUM_POINTS_IN_PROFILES)];
        float denom = p1.x_ - p2.x_;
        if(denom == 0.0f) denom = 3.0f*FLT_MIN;
        float a = (p1.y_ - p2.y_) / denom;
        float b = p1.y_ - a*p1.x_;
