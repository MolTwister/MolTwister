...
private:
    ...
    HOSTDEV_CALLABLE int calcIndexOfLowerPointOfInterpolationLine(const float& x, const CMDFFMatrices::CPoint& firstPointInList, const CMDFFMatrices::CPoint& lastPointInList);
    HOSTDEV_CALLABLE float calcAngle(const C3DVector& r_k, const C3DVector& r_i, const C3DVector& r_j);
    HOSTDEV_CALLABLE float calcDihedral(const C3DVector& r_k, const C3DVector& r_i, const C3DVector& r_j, const C3DVector& r_l);
    HOSTDEV_CALLABLE float interpolateLinear(const float& r, const float& x1, const float& y1, const float& x2, const float& y2);
